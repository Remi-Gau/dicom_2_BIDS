function varargout = nii_xform(src, target, rst, intrp, missVal)
    ...
    if nargin < 4 || isempty(intrp)
        intrp = 'linear';
    end
    if nargin < 5 || isempty(missVal)
        missVal = nan;
    end
    intrp = lower(intrp);
    ...
    elseif iscell(target)
        ...
        if strcmpi(ext, '.mat') % template and xform file names
            ...
            if ~isequal(size(R), [4 4])
                error('Invalid transformation file.');
            end
    ...
    for i = 1:numel(flds)
        nii.hdr.(flds{i}) = hdr.(flds{i});
    end

    if ~isempty(rst)
        nii_tool('save', nii, rst);
    end
    if nargout || isempty(rst)
