function [s, info, dict] = dicm_hdr(fname, dict, iFrames)
    ...
    for nb = [0 2e6 20e6 fSize] % if not enough, read more till all read
        ...
        for k = i(end:-1:1) % last is likely real PixelData
            ...
            if numel(b8) < p.iPixelData
                b8 = [b8 fread(fid, 8, '*uint8')'];
            end %#ok
            p.bytes = ch2int32(b8(p.iPixelData + (-3:0)), p.be);
            if p.bytes == 4294967295 && feof(fid)
                break
            end % 2^32-1 compressed
        ...
        if feof(fid)
            if isempty(i)
                p.iPixelData = fSize + 1;
            end % fake it: no PixelData
            break
        end
    end
    ...
    if toSearch % search each tag if header is short and not many tags asked
        if ~isempty(tsUID)
            s.TransferSyntaxUID = tsUID;
        end % hope it is 1st tag
        ...
        for k = numel(p.dict.tag):-1:1 % reverse order so reduce range each loop
            ...
            if hasVR
                vr = char(b8(i + (0:1)));
                i = i + 2;
                if strcmp(vr, 'UN') || strcmp(vr, 'OB')
                    vr = p.dict.vr{k};
                end
            else
                vr = p.dict.vr{k};
            end
            [n, nvr] = val_len(vr, b8(i + (0:5)), hasVR, swap);
            i = i + nvr;
            if n == 0
                continue
            end % dont assign empty tag

            [dat, info] = read_val(b8(i + (0:n - 1)), vr, swap);
            if ~isempty(info)
                toSearch = false;
                break
            end % re-do in regular way
            if ~isempty(dat)
                s1.(p.dict.name{k}) = dat;
            end
        end
    ...
    while ~toSearch
        ...
        if ~p.fullHdr && tg > p.dict.tag(end)
            break
        end % done for partial hdr
        if isempty(dat) || isnumeric(name)
            continue
        end
        s.(name) = dat;
...
function [dat, name, info, i, tag] = read_item(b8, i, p)
    ...
    if ~isempty(ind)
        ...
        if strcmp(vr, 'UN') || strcmp(vr, 'OB') || ~hasVR
            vr = p.dict.vr{ind};
        end
    ...
    elseif tag == 593936 % 0x0009 0010 GEIIS not dicom compliant
        i = i + n;
        return  % seems n is not 0xffffffff
    elseif p.fullHdr
        if elmnt == 0
            i = i + n;
            return
        end % skip GroupLength
        if mod(group, 2)
        ...
        if ~hasVR
            vr = 'UN';
        end % not in dict, will leave as uint8
    elseif n < 4294967295 % no skip for SQ with length 0xffffffff
        i = i + n;
        return
    end
    % compressed PixelData, n can be 0xffffffff
    if ~hasVR && n == 4294967295
        vr = 'SQ';
    end % best guess
    if n + i > p.iPixelData && ~strcmp(vr, 'SQ')
        i = i + n;
        return
    end % PixelData or err
...
function [rst, info, i] = read_sq(b8, i, nEnd, p, isPerFrameSQ)
    ...
    while i < nEnd % loop through multi Item under the SQ
        ...
        if tag ~= 4294893568
            i = i + 4;
            return
        end % only do FFFE E000, Item
        n = ch2int32(b8(i + (0:3)), p.be);
        i = i + 4; % n may be 0xffff ffff
        n = min(i + n, nEnd);
        j = j + 1;
        ...
        if isPerFrameSQ
            if ischar(p.iFrames) % 'all' frames
                if j == 1 && ~isnan(p.nFrames)
                    rst.FrameStart = nan(1, p.nFrames);
                end
        ...
        while i < n % loop through multi tags under one Item
            ...
            if tag == 4294893581
                break
            end % FFFE E00D ItemDelimitationItem
            if isempty(tag1)
                tag1 = tag;
            end % first detected tag for PerFrameSQ
            if isempty(dat) || isnumeric(name)
                continue
            end % 0-length or skipped
            rst.(Item_n).(name) = dat;
        end
    end
end
...
function [n, nvr] = val_len(vr, b, expl, swap)
    ...
    if n == 13
        n = 10;
    end % ugly bug fix for some old dicom file
end
...
function p = get_nFrames(s, p)
    ...
    elseif all(isfield(s, {'Columns' 'Rows' 'BitsAllocated'})) && p.bytes < 4294967295
        if isfield(s, 'SamplesPerPixel')
            spp = double(s.SamplesPerPixel);
        else
            spp = 1;
        end
...
function csa = read_csa(csa)
    b = csa';
    if numel(b) < 4 || ~strcmp(char(b(1:4)), 'SV10')
        return
    end % no op if not SV10
    chDat = 'AE AS CS DA DT LO LT PN SH ST TM UI UN UT';
    i = 8; % 'SV10' 4 3 2 1
    try %#ok in case of error, we return the original csa
        nField = ch2int32(b(i + (1:4)), 0);
        i = i + 8;
        for j = 1:nField
            i = i + 68; % name(64) and vm(4)
            vr = char(b(i + (1:2)));
            i = i + 8; % vr(4), syngodt(4)
            n = ch2int32(b(i + (1:4)), 0);
            i = i + 8;
            if n < 1
                continue
            end % skip name decoding, faster
            ...
            for k = 1:n % n is often 6, but often only the first contains value
                len = ch2int32(b(i + (1:4)), 0);
                i = i + 16;
                if len < 1
                    i = i + (n - k) * 16;
                    break
                end % rest are empty too
...
function ch = read_ProtocolDataBlock(ch)
    ...
    if ~all(ch(5:6) == [31 139]') || n > numel(ch)
        return
    end % gz signature
    ...
    if isempty(b{1})
        return
    end % guzip faild or wrong format

    try %#ok
        for j = 1:numel(b)
            a = sscanf(b{j}{2}, '%f', 1);
            if isempty(a)
                rst.(b{j}{1}) = strtrim(b{j}{2});
            else
                rst.(b{j}{1}) = a; % convert into num if possible
            end
        end
        ch = rst;
    end
...
function s1 = search_MF_val(s, s1, iFrame)
    ...
    if ~isfield(s, 'PerFrameFunctionalGroupsSequence')
        return
    end
    expl = false;
    be = false;
    ...
    for i = 1:numel(flds)
        ...
        if isempty(ind) % no tag in PerFrameSQ, try tag before PerFrameSQ
            ...
            if ~isempty(ind)
                ...
                [n, nvr] = val_len(vr, uint8(b0(k + (0:5))), isEX, isBE);
                k = k + nvr;
        ...
        for k = 1:nf
            ...
            if isempty(j)
                continue
            end % no tag for this frame
            if len == 2
                n = ch2int16(b(ind(j) + (0:1)), isBE);
            else
                n = ch2int32(b(ind(j) + (0:3)), isBE);
            end
            a = b(ind(j) + len + (0:n - 1));
            if isDS
                ...
                end % ignore in case of error
            elseif isCH
                while ~isempty(a) && a(end) == 0
                    a(end) = '';
                end
                try
                    s1.(flds{i}){k} = deblank(a);
                catch
                end
            else
...
function [s, err] = philips_par(fname)
    ...
    i1 = strfind(str, 'IMAGE INFORMATION DEFINITION');
    i1 = i1(end);
    ...
    i1 = strfind(str, '= IMAGE INFORMATION =');
    i1 = i1(end);
    ...
    for i = 1:9
        ...
        if ~isempty(foo)
            break
        end % get the first number
    end
    while str(ind(i)) == 10
        i = i + 1;
    end % skip empty lines (only one)
...
function [s, err] = afni_head(fname)
    ...
    if strcmp(foo(1), 'M')
        err = 'BYTEORDER_STRING not supported';
        s = [];
        return
    end
    ...
    if any(diff(foo) ~= 0)
        err = 'Inconsistent BRICK_FLOAT_FACS';
        s = [];
        return
    end
    if foo(1) == 0
    ...
    if ~isempty(i)
        s.ScanningSequence = strtok(hist(i:end), ' ');
    end
    i = strfind(hist, 'Studyid:') + 8;
    if ~isempty(i)
        s.StudyID = strtok(hist(i:end), ' ');
    end
    ...
    i = strfind(hist, 'TE:') + 3;
    if ~isempty(i)
        s.EchoTime = sscanf(hist(i:end), '%g', 1) * 1000;
    end
        ...
        if bv.TimeResolutionVerified
            switch bv.SliceAcquisitionOrder % the same as NIfTI?
                ...
                case 3
                    ind = [1:2:nSL 2:2:nSL];
                case 4
                    ind = [nSL:-2:1 nSL - 1:-2:1];
                case 5
                    ind = [2:2:nSL 1:2:nSL];
                case 6
                    ind = [nSL - 1:-2:1 nSL:-2:1];
                otherwise
                    ind = [];
                    err = 'Unknown SliceAcquisitionOrder';
            end
            if ~isempty(ind)
