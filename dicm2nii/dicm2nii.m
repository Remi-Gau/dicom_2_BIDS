function varargout = dicm2nii(src, niiFolder, fmt)
    ...
    % 131021 Check SamplesPerPixel, skip series if it is 1+.
    ...
    if ~isempty(unzip_cmd)
        ...
        if strcmp(unzip_cmd, 'unzip')
            ...
            if err
                unzip(src, dcmFolder);
            end % Matlab's unzip is too slow
    ...
    for i = 1:nRun
        ...
        if ~isempty(nSL) && nSL > 1
            ...
            if s.isDTI
                continue
            end % allow missing directions for DTI
            a = zeros(1, nFile);
            for j = 1:nFile
                a(j) = tryGetField(h{i}{j}, 'InstanceNumber', 1);
            end
            if any(diff(a) ~= 1)
        ...
        if ~keep(i) || nFile < 2 || ~isfield(s, 'ImagePositionPatient')
            continue
        end
        ...
        iSL = iSL(3);
        for j = 1:nFile
            ipp(j, :) = h{i}{j}.ImagePositionPatient(iSL);
        end
        [err, nSL, sliceN, isTZ] = checkImagePosition(ipp);
        if ~isempty(err)
    ...
    h = h(ind);
    sNs = sNs(ind);
    studyIDs = studyIDs(ind); % by subjs now
    ...
    for i = 1:nRun
        ...
        if i > 1 && sN - sNs(i - 1) == 1 && isType(s, '\MOCO\')
            a = [a '_MoCo'];
        end
        if multiSubj
            a = [a '_' subjs{i}];
        ...
        if nStudy(i) > 1
            a = [a '_Study' studyIDs{i}];
        end
        if ~isstrprop(a(1), 'alpha')
            a = ['x' a];
        end % genvarname behavior
    ...
    if nargout > 0
        varargout{1} = subj;
    end % return converted subject IDs
    if nargout > 1
        varargout{2} = {};
    end % will remove in the future
    ...
    subjStr = sprintf('''%s'', ', subj{:});
    subjStr(end + (-1:0)) = [];
    vendor = sprintf('%s, ', vendor{:});
    vendor(end + (-1:0)) = [];
    ...
    for i = 1:nRun
        ...
        if ~isfield(s, fld) && numel(dim) > 3 && dim(4) > 1
            h{i}{1}.(fld) = dim(4);
        end
        ...
        if isfield(s, 'RTIA_timer') && ~s.isDTI
            t = zeros(dim(3), 1);
            for j = 1:dim(3)
                t(j) = tryGetField(h{i}{j}, 'RTIA_timer', nan);
            end
            if ~all(diff(t) == 0)
                h{i}{1}.RefAcqTimes = t / 10;
            end % in ms
        ...
        if isnumeric(h{i}.PixelData)
            h{i} = rmfield(h{i}, 'PixelData');
        end % BV
    end
...
function subj = PatientName(s)
    ...
    if isempty(subj)
        subj = tryGetField(s, 'PatientID', 'Anonymous');
    end
...
function tf = isType(s, keyword)
    ...
    tf = ~isempty(strfind(typ, keyword));
...
function [nii, s] = set_nii_header(nii, s, pf)
    ...
    if flp(iSL) && isfield(s, 'SliceTiming') % slices flipped
        ...
        if sc > 0
            nii.hdr.slice_code = sc + mod(sc, 2) * 2 - 1;
        end % 1<->2, 3<->4, 5<->6
    end
    ...
    if isempty(seq)
        ...
        ind = strfind(seq, '\');
        if ~isempty(ind)
            seq = seq(ind(end) + 1:end);
        end % like 'ep2d_bold'
    end
    if pf.save_patientName
        nii.hdr.db_name = PatientName(s);
    end % char[18]
...
function vol = mos2vol(mos, nSL)
    ...
    nr = nr / nMos;
    nc = nc / nMos; % number of row and col in slice
...
function [s, hdr] = sliceTiming(s, hdr)
    ...
    if isempty(TR)
        TR = tryGetField(s, 'TemporalResolution');
    end
    if isempty(TR)
        return
    end
    hdr.pixdim(5) = TR / 1000;
    if tryGetField(s, 'isDTI', 0)
        return
    end
    hdr.xyzt_units = 8; % seconds
    if hdr.dim(5) < 3
        return
    end % skip structual, fieldmap etc
    ...
    if isempty(t)
        t = tryGetField(s, 'RefAcqTimes');
    end % GE or Siemens non-mosaic
    ...
    elseif t1(1) == t1(2)
        sc = 7;
        t1 = unique(t1); % made-up code 7 for MB
    elseif abs(dif - dur) < 1e-3
        sc = 1; % ascending
    elseif abs(dif + dur) < 1e-3
        sc = 2; % descending
...
function [h, nii] = get_dti_para(h, nii)
    ...
    elseif isfield(s, 'PerFrameFunctionalGroupsSequence')
        ...
        s2 = struct('B_value', nan(1, nDir), 'DiffusionGradientDirection', nan(3, nDir));
...
function save_dti_para(s, fname)
    if ~isfield(s, 'bvec') || all(s.bvec(:) == 0)
        return
    end
    if isfield(s, 'bval')
...
function [ixyz, R, pixdim, xyz_unit] = xform_mat(s, dim)
    ...
    if isempty(thk)
        thk = tryGetField(s, 'SliceThickness', pixdim(1));
    end
    pixdim = [pixdim(:); thk];
    ...
    if dim(3) < 2
        return
    end % don't care direction for single slice
...
function val = asc_header(s, key)
    ...
    i0 = i0(1) + numel(key);
    i1 = strfind(str(i0:end), char(10)); % next new line
    str = str(i0 + (1:i1(1) - 2)); % like '   = 0xee'
    i0 = strfind(str, '=');
    str = strtrim(str(i0(1) + 1:end)); % '0xee'
...
function gui_callback(h, evt, cmd, fh)
    ...
    switch cmd
        ...
        case 'set_src'
            ...
            if ~isempty(ind)
                return
            end % no check with multiple files
...
function create_gui
    ...
    if strcmp('dicm2nii_fig', get(fh, 'Tag'))
        return
    end
    ...
    sz = get(hs.gzip, 'Extent');
    set(hs.gzip, 'Position', [220 166 sz(3) + 16 sz(4)]);

    hs.rst3D = chkbox(fh, getpf('rst3D', false), 'SPM 3D', cb('SPMStyle'), ...
                      'Save one file for each volume (SPM style)');
    sz = get(hs.rst3D, 'Extent');
    set(hs.rst3D, 'Position', [330 166 sz(3) + 16 sz(4)]);
...
function [phPos, iPhase] = phaseDirection(s)
    ...
    if isfield(s, fld)
        if     strncmpi(s.(fld), 'COL', 3)
            iPhase = 2; % based on dicm_img(s,0)
        elseif strncmpi(s.(fld), 'ROW', 3)
            iPhase = 1;
        else
            errorLog(['Unknown ' fld ' for ' s.NiftiName ': ' s.(fld)]);
        end
    end
    ...
    elseif strncmpi(s.Manufacturer, 'Philips', 7)
        ...
        if R(ixy(iPhase), iPhase) < 0
            phPos = ~phPos;
        end % tricky! in image ref
    end
...
function s = multiFrameFields(s)
    ...
    if any(~isfield(s, {'SharedFunctionalGroupsSequence' pffgs}))
        return
    end
    ...
    if ~isfield(s, 'EchoTime')
        ...
        if ~isempty(a)
            s.EchoTime = a;
        elseif isfield(s, 'EchoTimeDisplay')
    ...
    if isempty(nFrame)
        nFrame = numel(s.(pffgs).FrameStart);
    end
    ...
    if ~isempty(i)
        ...
        if isequal(i, [1 nSL]) || isequal(i, [nSL 1])
            return
        end % not 100% safe
    end
...
function val = MF_val(fld, s, iFrame)
    ...
    pffgs = 'PerFrameFunctionalGroupsSequence';
    if nargin < 2
        val = {'SharedFunctionalGroupsSequence' pffgs sq fld};
        return
    end
    try
...
function [nii, niiP] = split_philips_phase(nii, s)
    ...
    if ~isfield(s, 'VolumeIsPhase') % PAR file and single-frame file have this
        dim = nii.hdr.dim(4:5);
        if tryGetField(s, 'Dim3IsVolume')
            iFrames = 1:dim(2);
        else
            iFrames = 1:dim(1):dim(1) * dim(2);
        end
...
function s = csa2pos(s, nSL)
    ...
    if all(sNormal == 0)
        return
    end % likely no useful info, give up
    ...
    if ~isfield(s, 'ImagePositionPatient')
        ...
        if all(pos(:, 2) == 0) % pos(:,1) for volume center
            if any(~isfield(s, {'PixelSpacing' 'SpacingBetweenSlices'}))
                return
            end
        ...
        if isMos
            pos(:, 2) = pos(:, 1);
        end % set LastFile same as first for mosaic
...
function t = mb_slicetiming(s, TA)
    ...
    if isempty(mb) || mb == 1 || mod(nSL, mb) > 0
        return
    end % not MB or wrong mb guess
...
function [err, nSL, sliceN, isTZ] = checkImagePosition(ipp)
    a = diff(sort(ipp));
...
function checkUpdate(mfile)
    ...
    try
        ...
        latestStr = regexp(str(ind(end):end), '<td>(.{10})</td>', 'tokens', 'once');
        if isempty(latestStr)
            error('Date info not found');
        end
...
function nMos = nMosaic(s)
    ...
    if ~isempty(nMos)
        return
    end % seen 0 for GLM Design file and others
    ...
    b = img(dim(1) + [-1 0], dim(2) + [-1 0]); % 2x2 at bottom-right
    done = false;
    if all(~b(:)) % at least 1 padded slice: not 100% safe
        t = img(1:2, dim(2) + [-1 0]); % 2x2 at top-right
        if all(~t(:)) % all right tiles padded: use all to determine
            ln = sum(img);
        else % use several rows at buttom to determine: not as safe as all
            ln = sum(img(dim(1) + [-3 0], :)); % assume slice dim>=4
        end
        z = find(ln ~= 0, 1, 'last');
        nMos = dim(2) / (dim(2) - z);
        done = mod(nMos, 1) == 0 && mod(dim(1), nMos) == 0;
    end
    if ~done % no padded slice or error in previous method
        % This relies on zeros at phase dir. May not work for all mosaic
        ...
        if sum(ln) < 2 % InPlanePhaseEncodingDirection: ROW
            ...
            ln(i + 2:end) = []; % leave only 1 true for padded slices
        end
        nMos = sum(ln);
    ...
    nMos = nMos * nMos;
    img = mos2vol(uint8(img), nMos); % find padded zeros: needed for STC
    while 1
        a = img(:, :, nMos);
        if any(a(:) ~= 0)
            break
