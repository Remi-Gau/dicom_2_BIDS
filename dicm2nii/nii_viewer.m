function varargout = nii_viewer(fname, overlayName)
    ...
    % provides a way to turn on/off an overlay back and forth to view the overlap.
    % Most operations are applied to the selected NIfTI in the list, such as Show
    % NIfTI hdr/ext under Window menu, Move/Close overlay under Overlay menu, and
    % most operations under File menu.
    %
    ...
    if siz(2) > 800
        siz = siz * 0.8;
    end % make it smaller on big screen
    siz = ceil(siz);
    ...
    if pos(1) + siz(1) > res(1)
        pos(1) = res(1) - siz(1) - 10;
    end
    if pos(2) + siz(2) > res(2) - 130
        pos(2) = min(pos(2), 50);
    end
    ...
    for i = 1:3
        ...
        hs.hsI(i) = image(zeros(dim(j([2 1])), 'single'));
        hold(hs.ax(i), 'on');
    ...
    try
        set([hs.ras hs.cross(:)' hs.xyz], 'PickableParts', 'none'); % new matlab
    catch
        set([hs.ras hs.cross(:)' hs.xyz], 'HitTest', 'off'); % old ones
    end
    ...
    uimenu(h_saveas, 'Label', 'file with a new resolution', 'Callback', @save_nii_as, 'Separator', 'on');
    ...
    try
        java_dnd(jFrame, {@javaDropFcn fh});
    catch me
        disp(me.message);
    end
    ...
    if pf.mouseOver
        set(fh, 'WindowButtonMotionFcn', cb('mousemove'));
    end
    if pf.rightOnLeft
    ...
    if nargin > 1
        ...
        elseif iscellstr(overlayName)
            for i = 1:numel(overlayName)
                addOverlay(overlayName{i}, fh);
            end
        end
    end
...
function nii_viewer_cb(h, ~, cmd, fh)
    ...
    switch cmd
        case 'ijk'
            ...
            xyz = set_xyz(hs);
            for i = 1:3
                set(hs.xyz(i), 'String', xyz(i));
            end % need 3 if oblique
        ...
        case {'lb' 'ub' 'lut' 'alpha' 'smooth' 'interp' 'volume'}
            ...
            if strcmp(cmd, 'lut')
                ...
                elseif any(val == 26:28) % error check for phase img
                    ...
                    if err
                        warndlg('Seleced image is not complex data.');
                    end
                elseif val == 29 % RGB
                    err = size(hs.q{i}.nii.img, 4) ~= 3;
                    if err
                        errordlg('RGB LUT requres 3-volume data.');
                    end
                end
                if err
                    hs.lut.Value = p(i).lut;
                    return
                end % undo selection
            end
        ...
        case 'resize'
            ...
            if isequal(oldF, posF(3:4))
                return
            end % moving without size change
        ...
        case 'open'
            ...
            fname = fullfile(pName, fname);
            if strcmp(get(h, 'Label'), 'Open in new window')
                nii_viewer(fname);
            else
                nii_viewer(fname, fh);
            end
        case 'add'
            ...
            if strcmp(label, 'Add aligned overlay')
                ...
                fname = cellstr(fname);
                for i = 1:numel(fname)
                    addOverlay(fullfile(pName, fname{i}), fh);
                end
            end
        ...
        case {'hdr' 'ext' 'essential'}
            ...
            elseif strcmp(cmd, 'ext')
                ...
                for i = 1:numel(hs.q{j}.nii.ext)
                    if ~isfield(hs.q{j}.nii.ext(i), 'edata_decoded')
                        continue
                    end
        ...
        case 'stack'
            ...
            switch get(h, 'Tag') % for both uimenu and pushbutton
                ...
                case 'down' % one level down
                    ...
                    for j = 1:3
                        uistack(p(i).hsI(j), 'down');
                    end
                    ind = [1:i - 1 i + 1 i i + 2:n];
                    i = i + 1;
                case 'top'
                    step = i - 1;
                    if step == 0
            ...
            p = p(ind);
            for j = 1:numel(p)
                hs.files.getModel.set(j - 1, str{j});
            end
            hs.files.selectNone; % seems unnecessary
            on = find([p.show]);
            if ~isempty(on)
                hs.files.setCheckBoxListSelectedIndices(on - 1);
            end
        ...
        case 'tc' % time course
            ...
            for j = 3:-1:1
                c(j) = get(hs.ijk(j), 'Value');
            end % ijk for background
        ...
        case 'hist'
            ...
            if n == nu
                ...
                edges = linspace(0, 1, n) * double(img(end) - img(1)) + double(img(1));
            end
            nam = strtok(hs.files.getModel.get(i - 1), '(');
            fh1 = figure(mod(fh.Number, 10) + i);
...
function set_zoom(m, hs)
    c = hs.dim(:) / 2;
    if m <= 1
        I = c; % full view regardless of crosshair location
    else
        I = cell2mat(get(hs.ijk, 'Value'));
    end
...
function KeyPressFcn(fh, evt)
    ...
    if any(strcmp(evt.Key, evt.Modifier))
        return
    end % only modifier
    hs = guidata(fh);
...
function javaDropFcn(~, evt, fh)
    try
        ...
        if strcmp(evt.DropType, 'file')
            ...
            fname = strtrim(evt.Data); % string
        end
        nii_tool('hdr', fname); % error out if invalid nii
...
function set_cdata(hs, iaxis)
    ...
    for i = 1:numel(p)
        if ~p(i).show
            continue
        end % save time, but need to update when enabled
        lut = p(i).lut;
        if lut == 11 % "lines" special case: do it separately
            vector_lines(hs, i, iaxis);
            continue
        ...
        for ix = iaxis
            ...
            if     ix == 1
                im = permute(im, [3 2 4 1]);
            elseif ix == 2
                im = permute(im, [3 1 4 2]);
            elseif ix == 3
                im = permute(im, [2 1 4 3]);
            end

            if ~isRGB % not NIfTI RGB
                rg = sort([p(i).lb p(i).ub]);
                if lut == 29 && hs.q{i}.nii.hdr.datatype == 2
                    rg = [0 255];
                end % uint8
...
function addOverlay(fname, fh, mtx)
    ...
    if any(q.nii.hdr.datatype == [32 1792]) % complex
        p(1).lut = 26; % phase
        p(1).lb = str2double(sprintf('%.2g', rg(2) / 2));
    elseif q.nii.hdr.intent_code == 1002 % Label
        p(1).lut = 24; % prism
    elseif q.nii.hdr.intent_code > 0 % some stats
        if rg(1) < 0
            p(1).lb = str2double(sprintf('%.2g', rg(2) / 2));
    ...
    if aligned
        niiName = [niiName '(aligned)'];
    end
    if singleVol
        niiName = [niiName '(' num2str(singleVol) ')'];
    end
...
function [q, frm, rg, dim] = read_nii(fname, ask_code, reOri)
    ...
    if q.nii.hdr.scl_slope == 0
        q.nii.hdr.scl_slope = 1;
    end
...
function vector_lines(hs, i, iaxis)
    ...
    for ix = iaxis
        ...
        if isfield(hs.q{i}, 'ivec')
            ...
            X = hs.q{i}.ivec(:, :, :, j(1));
            X = permute(X(ind), [j([2 1]) ix]);
            Y = hs.q{i}.ivec(:, :, :, j(2));
            Y = permute(Y(ind), [j([2 1]) ix]);
        else
            ii = {':' ':' ':'};
            ii{ix} = I;
            im = img(ii{:}, j);
...
function map = lut2map(lut, hs)
    ...
    elseif lut == 7
        map(:, 1) = 0; % cyan
    elseif any(lut == [8 19 26])
        map(:, 3) = 0;
        map(:, 1) = 1; % red_yellow
    elseif lut == 9
        map(:, 1) = 0;
        map(:, 3) = map(end:-1:1, 3); % blue_green
    elseif lut == 10 % two-sided
        ...
        map(:, 1) = 1; % red_yellow
        map_neg(:, 1) = 0;
        map_neg(:, 3) = map_neg(end:-1:1, 3); % blue_green
        map = [map_neg(end:-1:1, :); map];
    elseif lut == 11
        map(:, 2:3) = 0; % vector lines
    ...
    elseif lut == 28 % phase6: red-yellow-green/violet-blue-cyan
        ...
        map(33:48, 2) = 0;
        map(33:48, 3) = 1;
        map(33:48, 1) = a(64:-4:1);
        map(49:64, 1) = 0;
        map(49:64, 3) = 1;
        map(49:64, 2) = a(1:4:64);
    elseif lut == 29 % RGB
    else % custom
        map = hs.lutStr{lut};
    end
...
function out = smooth23(in, method, n, varargin)
    if size(in, 3) > 1
        out = smooth3(in, method, n, varargin{:});
        return
    end
...
function s = nii_essential(q)
    ...
    if isfield(q, 'mask_info')
        s.MaskedBy = q.mask_info;
    end
    if isfield(q, 'modulation_info')
        s.ModulatdBy = q.modulation_info;
    end
    if isfield(q, 'alignMtx')
        s.AlignMatrix = q.alignMtx;
    end
    switch hdr.intent_code
        case 2
            s.intent = 'Correlation';
            s.DoF = hdr.intent_p1;
        case 3
            s.intent = 'T-test';
            s.DoF = hdr.intent_p1;
        case 4
            s.intent = 'F-test';
            s.DoF = [hdr.intent_p1 hdr.intent_p2];
        case 5
            s.intent = 'Z-score';
        case 6
            s.intent = 'Chi squared';
            s.DoF = hdr.intent_p1;

            % several non-statistical intent_code
        case 1002
            s.intent = 'Label'; % e.g. AAL labels
        case 2003
            s.intent = 'RGB'; % triplet in the 5th dimension
        case 2004
            s.intent = 'RGBA'; % quadruplet in the 5th dimension
    end
    switch hdr.datatype
        case 0
        case 1
        ...
        case 16
            s.DataType = 'single';
        case 32
            s.DataType = 'single complex';
        case 64
            s.DataType = 'double';
        case 128
            s.DataType = 'uint8 RGB';
        case 256
            s.DataType = 'int8';
        case 511
            s.DataType = 'single RGB';
        case 512
            s.DataType = 'uint16';
        case 768
            s.DataType = 'uint32';
        ...
        case 1280
            s.DataType = 'uint64';
        case 1792
            s.DataType = 'double complex';
        case 2304
            s.DataType = 'uint8 RGBA';
        otherwise
            s.DataType = 'unknown';
    end
    ...
    switch bitand(hdr.xyzt_units, 7)
        case 1
            s.VoxelUnit = 'meters';
        case 2
            s.VoxelUnit = 'millimeters';
        case 3
            s.VoxelUnit = 'micrometers';
    end
    ...
    switch bitand(hdr.xyzt_units, 56)
        ...
        case 16
            s.TemporalUnit = 'milliseconds';
        case 24
            s.TemporalUnit = 'microseconds';
        case 32
            s.TemporalUnit = 'Hertz';
        case 40
            s.TemporalUnit = 'ppm';
        case 48
            s.TemporalUnit = 'radians per second';
    end
    ...
    switch hdr.slice_code
        case 0
        case 1
            s.SliceOrder = 'Sequential Increasing';
        case 2
            s.SliceOrder = 'Sequential Decreasing';
        case 3
            s.SliceOrder = 'Alternating Increasing 1';
        case 4
            s.SliceOrder = 'Alternating Decreasing 1';
        case 5
            s.SliceOrder = 'Alternating Increasing 2';
        case 6
            s.SliceOrder = 'Alternating Decreasing 2';
        otherwise
            s.SliceOrder = 'Multiband?';
    end
...
function r = img2mask(img)
    ...
    try
        ...
        while nc > 1 % +1 contours, put all into 1st
            ...
            if j == 1
                C = [C c{i}]; %#ok C(:,1) connect to c{i}(:,1)
            else
                C = [C c{i}(:, end:-1:1)]; %#ok C(:,end) to c{i}(:,end)
            end
            c(i) = [];
            nc = nc - 1;
        end
        ...
        ind = ye(1, :) == ye(2, :);
        xe(:, ind) = [];
        ye(:, ind) = []; % reomve horizontal edges
        minye = min(ye);
        maxye = max(ye);
        t = (ye == [minye; minye]);
        exminy = xe(:);
...
function set_colorbar(hs)
    if strcmpi(get(hs.colorbar, 'Visible'), 'off')
        return
    end
    p = hs.scroll.UserData(hs.files.getSelectedIndex + 1);
    if p.lut == 11
        map = lut2map(hs.lut.UserData, hs);
    else
        map = lut2map(p.lut, hs);
    end
    rg = sort([p.lb p.ub]);
    if any(p.lut == 26:28)
        labls = [0 180 360];
...
function set_file(~, evt, fh)
    if ~isempty(evt) && evt.getValueIsAdjusting
        return
    end
    hs = guidata(fh);
    i = hs.files.getSelectedIndex + 1;
...
function save_nii_as(h, ~)
    ...
    if ~isempty(strfind(c, 'dim 4')) % fsl RGB
        ...
        elseif ~any(nii.hdr.dim(5) == 3:4)
            errordlg('Selected image is not RGB data.');
            return
        end
    ...
    elseif ~isempty(strfind(c, '3D')) % SPM 3D
        nii = nii_tool('load', nam);
        if nii.hdr.dim(5) < 2
            errordlg('Selected image is not multi-volume data');
            return
        end
        [fname, pName] = uiputfile([pName '/*.nii'], ...
    ...
    elseif ~isempty(strfind(c, 'matching background'))
        ...
        fname = fullfile(pName, fname);
        pf = get(hs.pref, 'UserData');
        nii_xform(nam, hs.scroll.UserData(hs.iback).fname, fname, pf.interp, pf.extraV);
...
function java_robot(fh, action)
    ...
    if any(strcmp(action, 'key'))
        ...
        rob.keyPress(dummy);
        rob.delay(20);
        rob.keyRelease(dummy);
        pause(0.05);
        drawnow; % fire KeyPressFnc to set fh.UserData
    end
