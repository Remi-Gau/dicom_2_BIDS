function varargout = nii_tool(cmd, varargin)
    ...
    if strcmpi(cmd, 'init')
        if nargin < 2
            error('nii_tool(''%s'') needs second input', cmd);
        end
        for i = 1:size(C, 1)
            nii.hdr.(C{i, 1}) = C{i, 4};
        end
        ...
        if nargin > 2
            i = varargin{2};
            if i < 0 || i > 8 || mod(i, 1) > 0
                error('Invalid RGB_dim number');
            end
    ...
    elseif strcmpi(cmd, 'save')
        ...
        if nargin > 3 && ~isempty(varargin{3}) && varargin{3} && nii.hdr.dim(5) > 1
            ...
            for i = 1:nii.hdr.dim(5)
                ...
                if i == 1 && isfield(nii, 'ext')
                    nii0.ext = nii.ext;
                elseif i == 2 && isfield(nii0, 'ext')
                    nii0 = rmfield(nii0, 'ext');
                end
    ...
    elseif strcmpi(cmd, 'ext')
        ...
        if isempty(hdr.extension) || hdr.extension(1) == 0
            ...
            if hdr.vox_offset > 0
                nByte = hdr.vox_offset + 64; % .nii arbituary +64
            else
                nByte = inf;
            end
    ...
    elseif strcmpi(cmd, 'RGBStyle')
        ...
        if ischar(irgb)
            if strncmpi(irgb, 'fsl', 3)
                irgb = 4;
            elseif strncmpi(irgb, 'mricron', 4)
                irgb = 3;
            else
                irgb = 1;
            end
    ...
    elseif strcmpi(cmd, 'cat3D')
        ...
        for i = 2:n
            nii.img(:, :, :, i) = nii_tool('img', fnames{i});
        end
...
function cmd = check_gzip
    ...
    elseif ispc % rename back pigz for Windows. Renamed to trick Matlab Central
        ...
        if st && ~val.UserExecute
            fileattrib([m_dir '/pigz'], '+x');
        end
    end
...
function dd = check_dd
    ...
    if ~err
        dd = 'dd';
        return
    end % dd with linix/mac, and maybe windows
...
function outName = gunzipOS(fname, nByte)
    ...
    uid = [datestr(now, 'yymmddHHMMSSfff') sprintf('_%06x', randi(1e7))];
    ...
    if ~isempty(dd) && nargin > 1 && ~isinf(nByte) % unzip only part of data
        try
            n = num2str(ceil(nByte / 512)); % 512: default ibs
    ...
    if err
        fprintf(2, 'Error during decompression:\n%s\n', str);
    end
...
function hdr = read_hdr(b, C, fname)
    ...
    elseif n == 540
        ...
        elseif n == 540
            ...
            error('Not valid NIfTI file: %s', fname);
        end
    end

    if niiVer > 1
        C = niiHeader(niiVer);
    end % C defaults for version 1
    for i = 1:size(C, 1)
        try
            a = b(C{i, 5} + 1:C{i + 1, 5});
        catch
            if C{i, 5} == numel(b)
                ...
                a = b(C{i, 5} + (1:C{i, 2})); % last item extension is in bytes
            end
        end
...
function ext = read_ext(b, hdr)
    ...
    while j < nEnd
        esize = cast_swap(b(j + (1:4)), 'int32', swap);
        j = j + 4; % x16
        if isempty(esize) || mod(esize, 16)
            return
        end % just to be safe
        i = numel(ext) + 1;
        ext(i).esize = esize; %#ok<*AGROW>
        ext(i).ecode = cast_swap(b(j + (1:4)), 'int32', swap);
        j = j + 4;
        ...
        if ext(i).ecode == 40 % Matlab: any kind of matlab variable
            nByte = cast_swap(ext(i).edata(1:4), 'int32', swap); % num of bytes of MAT data
...
function fname = nii_name(fname, ext)
    ...
    if strcmpi(e, '.nii') || strcmpi(e, ext)
        return
    end
...
function subFuncHelp(mfile, subcmd)
    ...
    if isempty(ind)
        disp(str);
        return
    end % no topicChar found. Show all help text
    ...
    for i = 1:nTopic
        ...
        if isempty(ind)
            continue
        end % no syntax before ' - ', assume start with 1
...
function bytes = gunzip_mem(gz_bytes, fname, nByte)
    ...
    try
        ...
        try
            org.apache.commons.io.IOUtils.copy(gzis, buff);
        end %#ok
        gzis.close;
        bytes = typecast(buff.toByteArray, 'uint8'); % faster than uint8()
    catch
        if nargin < 3 || isempty(nByte)
            nByte = inf;
        end
