function img = dicm_img(s, xpose)
    ...
    if isstruct(s) && ~all(isfield(s, [flds 'PixelData']))
        s = s.Filename;
    end
    ...
    if nargin < 2 || isempty(xpose)
        xpose = true;
    end % same as dicomread by default
    ...
    if fid < 0
        ...
        if fid < 0
            error(['File not exists: ' s.Filename]);
        end
    end
    ...
    if ~isfield(s, 'TransferSyntaxUID') || ... % files other than dicom
        ...
        n = typecast(b(5:8), 'uint32');
        i = 8 + n; % length of offset table
        if n > 0
        ...
        for j = 1:nFrame
            ...
            n = typecast(b(i + uint32(1:4)), 'uint32');
            i = i + 4;
            if useMemmapfile
                m.Data(1:n) = b(i + (1:n));
                i = i + n;
